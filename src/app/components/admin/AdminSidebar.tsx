...
export function AdminSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-green-700 to-green-800 text-white transition-all duration-300 ease-in-out z-50 w-16 ${
    ...
    >
        ...
        {filteredMenuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {showLabels && <span className="hidden md:inline font-medium">{item.label}</span>}
            </button>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-green-600 space-y-2">
        {userName && (
          <div
            className={`flex items-center gap-3 px-3 py-2 rounded-lg bg-green-600/50 ${
              collapsed ? 'justify-center' : 'justify-center md:justify-start'
            }`}
        ...
        <button
          onClick={onLogout}
          className={`w-full flex items-center gap-3 px-3 py-3 rounded-lg text-white hover:bg-red-600 transition-colors ${
            collapsed ? 'justify-center' : 'justify-center md:justify-start'
          }`}
          title={collapsed ? 'Cerrar Sesión' : ''}
        >
          <LogOut className="w-5 h-5 flex-shrink-0" />
          {showLabels && <span className="hidden md:inline font-medium">Cerrar Sesión</span>}
        </button>
      </div>
    </aside>
  );
